...
import { mexcAccounts, fastCommands, autoCoins } from "../storage/schema";
...
export async function parseAndExecuteCommand(message: string, userId: string, mastra?: any): Promise<string> {
  ...
  if (cmd === "/start" || cmd === "/help") {
    const helpText = `🤖 *Multi Mexc Futures Bot*

*Принцип работы:*

Чтобы запустить бота команда /start
Бот всегда открывает на максимальное количество контрактов
Плечо - 20
Бот всегда открывает все активные аккаунты
Есть возможность открывать маркетом, bbo, лимитной лесенкой с разницей в 0,01%
Закривать можно маркетом и bbo

*Как начать?*

Нужно зарегистрировать аккаунты
Вводить команду вручную для открытия ордеров или добавить интересующие монеты в кнопку «Fast» и открывать позиции по маркету одной кнопкой
Закрываются позиции только командами или вручную

*Аккаунты можно добавлять и удалять
*Монеты в «Fast» можно добавлять и удалять
*Лимитные ордера на открытие шорта лесенкою открывають только на тех монетах где два нуля и меньше, пример: 0.003, где 3 нуля 0,0003 будет открывать лимитный ордер с одинаковой ценой на всех аккаунтах
*из-за того, что у мекса высокая нагрузка на серверах могут быть задержки с открытием
...
/fast add SYMBOL - добавить токен в быстрое открытие
/auto add SYMBOL dex_address - добавить монету с DEX адресом для автооткрытия
...
Используйте /register для добавления`;
  ...
  // Auto command - manage auto coins list with DEX addresses
  if (cmd === "/auto" || cmd === "🤖 auto") {
    try {
      const result = await db
        .select()
        .from(autoCoins)
        .where(eq(autoCoins.telegramUserId, userId))
        .limit(1);
      ...
      if (existing) {
        try {
          coins = JSON.parse(existing.coins || "[]");
        } catch (e) {
          coins = [];
        }
      }
      
      let text = `🤖 Auto команды\n\n`;
      const keyboard: any[][] = [];
      ...
      if (coins.length > 0) {
        ...
        text += `Нажми кнопку для открытия SHORT на эту монету\n\n`;
        text += `DEX адреса сохранены для каждой монеты\n`;
      } else {
        text += `Нет сохранённых монет\n\n`;
      }
  ...
  if (cmd.startsWith("auto_cmd_")) {
    ...
    try {
      ...
      if (index >= 0 && index < coins.length) {
        const coin = coins[index];
        const cmdToExecute = `/sm ${coin.symbol}`;
        return parseAndExecuteCommand(cmdToExecute, userId, mastra);
      } else {
        return `❌ Команда не найдена`;
      }
  ...
  // Handle add auto coin
  if (cmd.startsWith("/auto add ")) {
    const params = message.substring(9).trim().split(/\s+/);
    ...
    try {
      ...
      const alreadyExists = coins.some(c => c.symbol === symbol);
      if (!alreadyExists) {
        coins.push({ symbol, dex });
      }
      
      const coinsJson = JSON.stringify(coins);
      
      if (existing) {
        await db.update(autoCoins)
          .set({ coins: coinsJson, updatedAt: new Date() })
          .where(eq(autoCoins.telegramUserId, userId));
      } else {
        await db.insert(autoCoins).values({
          telegramUserId: userId,
          coins: coinsJson,
        });
      }
  ...
  // Callback handler for add auto coin
  if (cmd === "add_auto_coin") {
    return `✏️ Отправь монету и DEX адрес:\n\n/auto add WOJAKONX fdry5i5kuadz1ik8gps26qjj9rw9mpufxmeggc2hnsp7`;
  }
  ...
  // Show auto menu callback
  if (cmd === "show_auto") {
    return parseAndExecuteCommand("/auto", userId, mastra);
  }
  
  // Handle back button from auto menu - show keyboard menu
...
Используйте /register для добавления`;
  ...
  if (message === "🚀 Начало") {
    return await parseAndExecuteCommand("/start", userId, mastra);
  }
